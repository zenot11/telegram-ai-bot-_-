...
На 37 этапе `backend_stub` получил второй источник данных для вузов, на 38 этапе PostgreSQL-режим расширен справочниками, фильтрами и сортировкой, на 40 этапе пользовательские интерфейсы переведены на PostgreSQL-first поведение, на 41 этапе отполировано отображение реальных PostgreSQL-карточек, на 44 этапе уточнены релевантность, ранжирование, качество баллов и coverage audit, а на 45 этапе проведён полный аудит использования SQL-схемы команды:
...
- `/api/directions?q=...` ищет по полному PostgreSQL-справочнику направлений, кодов и профилей, даже если Mini App показывает только ограниченный стартовый список подсказок;
...
- ввод вида `09.03.04 Программная инженерия` сначала ищется по точному `directions.code`; fallback по `directions.name` и `directions.profile` включается только если точный код ничего не вернул.
...
`passing_scores.min_score` читается напрямую как `min_score`, `passing_scores.admission_type` как `admission_type`, `passing_scores.year` как `year`, `passing_scores.note` как `note`, `directions.study_form` как `study_form`, `faculties.name` как `faculty`. Значения `min_score < 40` не считаются полноценным проходным баллом: backend добавляет `score_is_valid=false`, для `1 < min_score < 40` также `score_is_suspicious=true`, а UI показывает уточняющую подпись и не считает от них запас.
...
- `directions`: `university_id`, `faculty_id`, `code`, `name`, `profile`, `study_form`;
...
Для бюджетных SQL-конкурсов также поддерживаются `target`, `special_quota`, `separate_quota` и `additional`: в API они остаются бюджетными по полю `type`, а в UI могут отображаться отдельной строкой `Конкурс`. Обычные `budget` и `paid` не показываются как дубль к строке `Финансирование`.

Полная таблица использования SQL-полей зафиксирована в [DATABASE_USAGE.md](DATABASE_USAGE.md). Там отдельно отмечено, почему `users`, `user_ege_scores`, `user_achievements`, `user_favorites` и view `v_directions_with_latest_budget` не используются текущим Python backend.
...
curl "http://localhost:8000/api/universities?region=Москва&city=Москва&score=276&direction=09.03.04%20Программная%20инженерия&study_form=заочная&type=paid&limit=20"
