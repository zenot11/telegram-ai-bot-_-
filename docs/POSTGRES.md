...
На 37 этапе `backend_stub` получил второй источник данных для вузов, на 38 этапе PostgreSQL-режим расширен справочниками, фильтрами и сортировкой, на 40 этапе пользовательские интерфейсы переведены на PostgreSQL-first поведение, на 41 этапе отполировано отображение реальных PostgreSQL-карточек, а на 43 этапе добавлены диагностика покрытия и поиск направлений по коду:
...
- ввод вида `09.03.04 Программная инженерия` ищется по `directions.code`, `directions.name` и `directions.profile`.
...
Для бюджетных SQL-конкурсов также поддерживаются `target`, `special_quota`, `separate_quota` и `additional`: в API они остаются бюджетными по полю `type`, а в UI могут отображаться отдельной строкой `Конкурс/квота`. Обычные `budget` и `paid` не показываются как дубль к строке `Финансирование`.
