...
- `styles.css` - стиль в духе командного сайта “Аиша”, CSS-переменные для светлой/тёмной темы, анимированный брендовый логотип без внешних шрифтов, адаптивные вкладки, фильтры, карточки, таблица сравнения, быстрый старт, контекстные подсказки, план поступления, экспортный отчёт, print CSS, форма поддержки и toast-уведомления.
...
Визуальный слой Mini App использует только HTML/CSS/JS: брендовый логотип Aisha появляется побуквенно через CSS-анимацию, а `prefers-reduced-motion` отключает новую анимацию для пользователей, которым она не подходит. Внешние шрифты, CDN, GIF, видео и Lottie не используются.
