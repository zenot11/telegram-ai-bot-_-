...
- `styles.css` - светлый стиль в духе командного сайта “Аиша”, адаптивные вкладки, фильтры и карточки.
- `app.js` - вкладки, валидация формы, запрос к `/api/universities`, локальные фильтры, итог подбора, избранное через `localStorage` и отрисовка карточек.
- `favicon.svg` - favicon.

Mini App не использует OpenAI и не хранит токены. Избранное Mini App хранится локально в браузере и не синхронизируется с Telegram-ботом, потому что авторизация и `initData`-проверка не входят в текущий этап.
