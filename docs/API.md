...
Поля `year`, `faculty`, `admission_type`, `admission_type_label`, `profile`, `direction_code`, `university_full_name`, `university_short_name`, `score_is_valid`, `score_display`, `score_note`, `source` могут присутствовать в PostgreSQL mode. Старые клиенты могут их игнорировать. `source` является диагностическим полем API и не показывается в обычном пользовательском интерфейсе.
