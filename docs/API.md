...
- `direction` - направление. Можно передавать название, профиль или код формата `09.03.04`; backend ищет код в `directions.code`, а текст - в названии и профиле.
...
- `09.03.04 Программная инженерия` -> поиск по `directions.code = 09.03.04` и тексту `Программная инженерия`;
...
Поля `year`, `faculty`, `admission_type`, `admission_type_label`, `financing_label`, `contest_label`, `study_form_label`, `profile`, `direction_code`, `university_full_name`, `university_short_name`, `score_is_valid`, `score_display`, `score_note`, `source` могут присутствовать в PostgreSQL mode. Старые клиенты могут их игнорировать. `source` является диагностическим полем API и не показывается в обычном пользовательском интерфейсе.

UI использует подписи:

- `financing_label` или `type` -> `Финансирование`;
- `study_form_label` или `study_form` -> `Форма обучения`;
- `contest_label`/`admission_type_label` -> `Конкурс/квота`, но общий бюджетный конкурс не дублируется как `Конкурс: бюджет`.

Правила отображения баллов:

- `min_score` остаётся числом или `null`; backend не заменяет его строкой.
- `score_is_valid=true`, если `min_score > 1`.
- `min_score=0`, `min_score=1`, `null` и отрицательные значения не показываются в UI как обычный проходной балл.
- Для таких записей UI показывает `Проходной балл: не указан` или `балл требует уточнения`, не считает `Запас` и не присваивает категорию `безопасный вариант` только из-за малого значения.
...
`note` используется для пометки демонстрационных данных или для контекста PostgreSQL-записи. Малые значения `min_score`, например `25`, не исправляются искусственно: UI показывает рядом `Конкурс`, `Год данных` и `Примечание`, чтобы не выдавать догадки за данные. Значения `0/1` считаются требующими уточнения и не используются для расчёта запаса.
