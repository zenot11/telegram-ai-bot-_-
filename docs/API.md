...
`backend_stub` - aiohttp backend API проекта “Аиша”. Он используется Telegram-ботом и Mini App; имя папки осталось историческим и не означает отдельный пользовательский тестовый слой.
...
- `limit` - максимальное количество результатов в текущем ответе, максимум 200. Это лимит выдаваемых карточек, а не размер базы.
...
- `note` - контекст источника или примечание к записи.
