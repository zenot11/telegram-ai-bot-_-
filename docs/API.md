...
`count` - число элементов в текущем ответе, `total_count` - полный размер справочника до frontend/endpoint-лимита. Mini App использует `total_count`, чтобы показывать `загружено 200 из 937`, если список направлений ограничен для `<select>`.
